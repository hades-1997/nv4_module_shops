...
if ($rowcontent['id'] > 0) {
    ...
    $rowcontent['catids'] = $db->query("SELECT listcatid FROM " . $db_config['prefix'] . "_" . $module_data . "_rows where id=" . $rowcontent['id'])->fetchColumn();

    $rowcontent['catids'] = $rowcontent['catids_old'] = !empty($rowcontent['catids']) ? array_map('intval', explode(',', $rowcontent['catids'])) : array();
}
...
if ($nv_Request->get_int('save', 'post') == 1) {
    ...
    $rowcontent['catids'] = $nv_Request->get_array('catids', 'post', []);
    ...
    if ($rowcontent['id'] == 0 or $is_copy) {
        ...
        $data_insert = [];
        $data_insert['array_catid'] = implode(',', $rowcontent['catids']);
        ...
        $stmt = $db->prepare("UPDATE " . $db_config['prefix'] . "_" . $module_data . "_rows SET
             array_catid='" . implode(',', $rowcontent['catids']) . "',
             listcatid= :listcatid,
    ...
    if ($rowcontent['catids'] != $rowcontent['catids_old']) {
        $sth = $db->prepare('INSERT INTO ' . $db_config['prefix'] . '_' . $module_data . '_rows_cats (rows_id, cats_id) VALUES( :rows_id, :cats_id )');

        foreach ($rowcontent['catids'] as $catids) {
            if (!in_array($catids, $rowcontent['catids_old'])) {
                $sth->bindParam(':rows_id', $rowcontent['id'], PDO::PARAM_INT);
                $sth->bindParam(':cats_id', $catids, PDO::PARAM_INT);
                $sth->execute();
            }
        }

        foreach ($rowcontent['catids_old'] as $catids_old) {
            if (!in_array($catids_old, $rowcontent['catids'])) {
                $db->query('DELETE FROM ' . $db_config['prefix'] . '_' . $module_data . '_rows_cats WHERE cats_id = ' . $catids_old . ' AND rows_id=' . $rowcontent['id']);
            }
        }
    }
...
$array_catid_in_row = explode(',', $rowcontent['array_catid']);

foreach ($global_array_shops_cat as $catid_i => $array_value) {
    $check_show = 1;
    ...
    $temp = array(
        ...
        'catidchecked' => ($catid_i == $rowcontent['listcatid']) ? ' checked="checked"' : '',
